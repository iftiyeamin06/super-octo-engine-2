...
import { api, type DesignationItem, type TenantListItem } from "../lib/api";
...
export default function Designations() {
  ...
  const [modal, setModal] = useState<{ open: boolean; editing?: DesignationItem }>({ open: false });
  ...
  const load = () => {
    ...
    api.designations.list(tenantFilter ? Number(tenantFilter) : undefined)
      .then(setItems).catch((e) => setError(e instanceof Error ? e.message : "Failed to load designations")).finally(() => setLoading(false));
  };
  ...
  async function save() {
    ...
    try {
      ...
      if (modal.editing) await api.designations.update(modal.editing.id, data);
      else await api.designations.create(data);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground">{items.length} designations</span>
      </div>
        ...
        <div className="bg-card border rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {items.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-10 text-center text-muted-foreground">No designations found</td></tr>
              ) : items.map((d) => (
                <tr key={d.id} className="hover:bg-muted/30 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-purple-500/10 text-purple-600 flex items-center justify-center flex-shrink-0"><Briefcase className="w-4 h-4" /></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-card border rounded-xl w-full max-w-md shadow-xl">
            <div className="flex items-center justify-between px-5 py-4 border-b">
              <h2 className="text-sm font-semibold">{modal.editing ? "Edit Designation" : "New Designation"}</h2>
            ...
            <div className="px-5 py-4 space-y-3">
              ...
              <div>
                ...
                <input value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Senior Engineer"
