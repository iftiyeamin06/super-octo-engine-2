...
export default function UserAccess() {
  ...
  const [loadErrors, setLoadErrors] = useState<{ modules?: string; roles?: string; users?: string; routes?: string }>({});
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      api.routes.list(),
    ]).then(([modsResult, rolesResult, usersResult, routesResult]) => {
      if (modsResult.status === "fulfilled") setModules(modsResult.value);
      ...
      if (routesResult.status === "fulfilled") setAllRoutes(routesResult.value);
      const errs: typeof loadErrors = {};
      if (modsResult.status === "rejected") errs.modules = "Failed to load modules";
      if (rolesResult.status === "rejected") errs.roles = "Failed to load roles";
      if (usersResult.status === "rejected") errs.users = "Failed to load users";
      if (routesResult.status === "rejected") errs.routes = "Failed to load routes";
      setLoadErrors(errs);
  ...
  function selectUser(user: UserListItem) {
    ...
    Promise.allSettled([
      api.users.moduleAccesses(user.id),
      api.users.routeAccesses(user.id),
    ]).then(([modulesResult, routesResult]) => {
      if (gen !== selectUserGen.current) return;
      if (modulesResult.status === "fulfilled") setDirectModuleIds(modulesResult.value);
      else setSaveErrors(p => ({ ...p, modules: "Failed to load module access" }));
      if (routesResult.status === "fulfilled") setDirectRouteIds(routesResult.value);
      else setSaveErrors(p => ({ ...p, routes: "Failed to load route access" }));
    }).finally(() => {
      if (gen === selectUserGen.current) setLoadingUser(false);
    });
  }
  ...
  async function saveAll() {
    ...
    if (!errors.roles && !errors.modules && !errors.routes) {
      ...
      const freshUser = await api.users.detail(selectedUser.id).catch(() => null);
      if (freshUser) {
      ...
      <div className="bg-card border rounded-xl p-4 space-y-3">
        <div className="relative" ref={dropdownRef}>
          ...
          {showUserDropdown && filteredUsers.length > 0 && (
            <div className="absolute left-0 right-0 top-full mt-1 bg-card border rounded-lg shadow-lg max-h-60 overflow-y-auto z-50">
          ...
          {showUserDropdown && filteredUsers.length === 0 && (
            <div className="absolute left-0 right-0 top-full mt-1 bg-card border rounded-lg p-4 text-sm text-muted-foreground text-center z-50">No users found</div>
          )}
        </div>
        {showUserDropdown && (
