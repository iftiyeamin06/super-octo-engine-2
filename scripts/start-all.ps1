...
@("CentralAuth API :5089", "CentralAuth UI :5173") | ForEach-Object {
    $title = $_
    $proc = Get-Process | Where-Object { $_.MainWindowTitle -eq $title } -ErrorAction SilentlyContinue
    if ($proc) {
        Write-Status "Closing window: $title"
...
for ($i = 0; $i -lt 20; $i++) {
    Start-Sleep -Seconds 1
    try {
        $null = Invoke-RestMethod "http://127.0.0.1:5089/health" -TimeoutSec 2
        $ready = $true
        break
    } catch {}
}
...
Write-Host "  |  API       ->  http://127.0.0.1:5089     |" -ForegroundColor Magenta
Write-Host "  |                                          |" -ForegroundColor Magenta
...
Write-Host "    - ""CentralAuth API :5089""" -ForegroundColor Gray
